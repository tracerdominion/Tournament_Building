  ...
  for (let i=0; i < stages.length; i++) {
    if ((admin[i+6][2]) && (!admin[i+6][3])) {
      switch (stages[i].type.toLowerCase()) {
        ...
        case 'double':
        case 'double elimination':
          found = updateDoubleBracket(i,true);
          break;
        case 'swiss':
        case 'swiss system':
...
function createGroups(stage) {
  ...
  switch (options[5][1].toLowerCase()) {
    case 'wins only':
      ...
      break;
    case 'separate':
      processing.getRange('T1').setFormula('=query(L:R, "select L,R,M,Q where not L=\'\' order by R, Q desc")');
      for (let i=0; i<options[3][1]; i++) {
        ...
        for (let j=0; j<ngroups; j++) {
          if (i == options[3][1]-1) {
            ...
            aggTable.push([openCheckString + (j+1) + ")",
              ...
              openCheckString + "index('" + stages[stage].name + " Standings'!A:A, 2+" + (4 + Number(options[3][1])) + "*'" + stages[stage].name + " Processing'!V" + (i*ngroups+j+2) + "))",
              openCheckString + "'" + stages[stage].name + " Processing'!W" + (i*ngroups+j+2) + ")"
            ]);
          } else {
            aggTable.push([j+1,
              "='" + stages[stage].name + " Processing'!T" + (i*ngroups+j+2),
              "='" + stages[stage].name + " Processing'!U" + (i*ngroups+j+2),
              "=index('" + stages[stage].name + " Standings'!A:A, 2+" + (4 + Number(options[3][1])) + "*'" + stages[stage].name + " Processing'!V" + (i*ngroups+j+2) + ")",
              "='" + stages[stage].name + " Processing'!W" + (i*ngroups+j+2)
            ]);
          } 
        }
        aggregate.getRange(1, 1, aggTable.length, 5).setValues(aggTable);
      ...
      break;
    case 'fixed':
      processing.getRange('T1').setFormula('=query(L:R, "select L,R,M,Q where not L=\'\' order by R, M")');
      var aggregate = sheet.insertSheet(stages[stage].name + ' Aggregation');
      var aggTable = [['Seed', 'Player', 'Place', 'Group', 'Win Percentage']];
      for (let i=0; i<ngroups; i++) {
        aggTable.push([i+1, "='" + stages[stage].name + " Processing'!T" + (i+2), "='" + stages[stage].name + " Processing'!U" + (i+2),
      ...
      for(let i=ngroups; i>0; i-=2) {
        aggTable.push([2*ngroups+1-i, "='" + stages[stage].name + " Processing'!T" + (ngroups+i), "='" + stages[stage].name + " Processing'!U" + (ngroups+i),
          "=index('" + stages[stage].name + " Standings'!A:A, 2+" + (4 + Number(options[3][1])) + "*'" + stages[stage].name + " Processing'!V" + (ngroups+i) + ")",
          "='" + stages[stage].name + " Processing'!W" + (ngroups+i)
        ]);
        aggTable.push([2*ngroups+2-i, "='" + stages[stage].name + " Processing'!T" + (ngroups+i+1), "='" + stages[stage].name + " Processing'!U" + (ngroups+i+1),
          "=index('" + stages[stage].name + " Standings'!A:A, 2+" + (4 + Number(options[3][1])) + "*'" + stages[stage].name + " Processing'!V" + (ngroups+i+1) + ")",
          "='" + stages[stage].name + " Processing'!W" + (ngroups+i+1)
        ]);
      }
      aggregate.getRange(1, 1, aggTable.length, 5).setValues(aggTable);
      break;
    ...
    default:
      ...
      for (let i=0; i<options[2][1]; i++) {
        aggTable.push([i+1,
          "='" + stages[stage].name + " Processing'!T" + (i+2),
          "='" + stages[stage].name + " Processing'!U" + (i+2),
          "=index('" + stages[stage].name + " Standings'!A:A, 2+" + (4 + Number(options[3][1])) + "*'" + stages[stage].name + " Processing'!V" + (i+2) + ")",
          "='" + stages[stage].name + " Processing'!W" + (i+2)
        ]);
      }
...
function updateDoubleBracket(stage, add) {
  var sheet = SpreadsheetApp.getActiveSpreadsheet();
  var form = FormApp.openByUrl(sheet.getSheetByName('Administration').getDataRange().getValues()[2][1]).getResponses();
  var mostRecent = form[form.length - 1].getItemResponses().map(function(resp) {return resp.getResponse()});
  var bracket = sheet.getSheetByName(stages[stage].name + ' Bracket');
  ...
  var ltop = 2**(nround + 1) + 2;
  var loserInfo = bracket.getRange(ltop - 1, 1, 1, 2).getValues();
  ...
  if (loserInfo[0][1]) {
    ...
    if (options[5][1].toLowerCase() != 'grand') {
      ...
      if ((match[0][0] == mostRecent[0]) && (match[1][0] == mostRecent[2])) {
        let topwins = Number(match[0][1]) + (add?1:-1)*Number(mostRecent[1]);
        let bottomwins = Number(match[1][1]) + (add?1:-1)*Number(mostRecent[3]);
      ...
      if ((match[0][0] == mostRecent[2]) && (match[1][0] == mostRecent[0])) {
        let topwins = Number(match[0][1]) + (add?1:-1)*Number(mostRecent[3]);
        let bottomwins = Number(match[1][1]) + (add?1:-1)*Number(mostRecent[1]);
        bracket.getRange(2**nround + 2, 2*nround + 5, 2, 1).setValues([[topwins],[bottomwins]]);
        if ((topwins >= gamesToWin) && (topwins > bottomwins)) {
          bracket.getRange(2**nround + 2, 2*nround + 4, 1, 2).setBackground('#D9EBD3');
        } else if ((bottomwins >= gamesToWin) && (bottomwins > topwins)) {
  ...
  } else if (loserInfo[0][0].indexOf('~' + mostRecent[0] + '~') == -1) {
    //in winner bracket
    for (let j=nround; j>0; j--) {
      let nmatches = 2**(nround-j);
      let space = 2**j;
      for (let i=0; i<nmatches; i++) {
        let match = bracket.getRange(space + 2*i*space, 2*j, 2, 2).getValues();
        if ((match[0][0] == mostRecent[0]) && (match[1][0] == mostRecent[2])) {
          var topwins = Number(match[0][1]) + (add?1:-1)*Number(mostRecent[1]);
          var bottomwins = Number(match[1][1]) + (add?1:-1)*Number(mostRecent[3]);
          found = true;
        } else if ((match[0][0] == mostRecent[2]) && (match[1][0] == mostRecent[0])) {
          var topwins = Number(match[0][1]) + (add?1:-1)*Number(mostRecent[3]);
          var bottomwins = Number(match[1][1]) + (add?1:-1)*Number(mostRecent[1]);
          found = true;
        }
        if (found) {
          bracket.getRange(space + 2*i*space, 2*j+1, 2, 1).setValues([[topwins],[bottomwins]]);
          if ((topwins >= gamesToWin) && (topwins > bottomwins)) {
            ...
            bracket.getRange(space + 2*i*space, 2*j, 1, 2).setBackground('#D9EBD3');
            bracket.getRange(ltop - 1, 1).setValue(loserInfo[0][0] + '~' + match[1][0] + '~');
            if (nlround % 2) {
              if (j>2) {
                bracket.getRange(ltop + 4 * (2**(j-3) - 1) - 2*j + 7 + ((j%2) ? i : -1-i+2**(nround-j))*8*2**(j-3), 4*j-6).setValue(match[1][0]);
              } else if (j == 2) {
            ...
            bracket.getRange(space + 2*i*space+1, 2*j, 1, 2).setBackground('#D9EBD3');
            bracket.getRange(ltop - 1, 1).setValue(loserInfo[0][0] + '~' + match[0][0] + '~');
            if (nlround % 2) {
              if (j>2) {
